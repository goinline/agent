...
func (p *runtimeBlock) Serialize() ([]byte, error) {
	...
	ret[00] = metricValue("GoRuntime/NULL/Goroutine", &p.NumGoroutine)
	ret[01] = metricValue("GoRuntime/NULL/CgoCall", &p.NumCgoCall)
	ret[02] = metricValue("GoRuntime/NULL/Frees", &p.Frees)
	ret[03] = metricValue("GoRuntime/NULL/Mallocs", &p.Mallocs)
	ret[04] = metricValue("GoRuntime/NULL/Lookups", &p.Lookups)
	ret[05] = metricValue("GC/NULL/Count", &p.NumGC)
	ret[06] = metricValue("GC/NULL/Time", &p.GCTime)
	ret[07] = metricValue("CPU/NULL/UserTime", &p.UserTime)
	ret[08] = metricValue("CPU/NULL/UserUtilization", &p.UserUtilization)
	ret[09] = metricValue("Memory/NULL/PhysicalUsed", &p.mem)
